...
angular.module('bw.paging', []).directive('paging', function () {
    ...
    function fieldTemplate(el, attrs) {
        return '<div class="col-lg-12">' +
            '<ul data-ng-hide="Hide" data-ng-class="ulClass" class="pagination pagination-rounded float-right"> ' +
            '<li ' +
            'title="{{Item.title}}" ' +
            'class="page-item" ' +
            ...
            '</ul>' +
            '<ul data-ng-hide="Hide" data-ng-class="ulClass" class="pagination pagination-rounded float-left" ng-if="total>0"> ' +
            '<li class="page-item" ng-if="page*pageSize>=total">' +
            '{{ page*pageSize-(pageSize-1)}} to {{total}} of {{total}}' +
            '</li>'
            + '<li class="page-item" ng-if="page*pageSize<total">' +
            '{{ page*pageSize-(pageSize-1)}} to {{ page*pageSize}} of {{total}}' +
            '</li>' +
            '</ul>' +
            '</div>'
    }
    ...
    function build(scope, attrs) {
        ...
        if (pageCount <= (fullAdjacentSize + 0)) {
